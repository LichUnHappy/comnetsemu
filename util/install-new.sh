...
function install_bcc() {
    ...
    if [[ $(lsb_release -rs) == "18.04" ]]; then # replace 18.04 by the number of release you want
	$install bison build-essential cmake flex git libedit-dev libllvm6.0 llvm-6.0-dev libclang-6.0-dev python zlib1g-dev libelf-dev
	elif [[ $(lsb_release -rs) == "19.04" ]]; then # replace 19.04 by the number of release you want
	# For Eon (19.10)
	$install bison build-essential cmake flex git libedit-dev libllvm7 llvm-7-dev libclang-7-dev python zlib1g-dev libelf-dev
	elif [[ $(lsb_release -rs) == "20.04" ]]; then # replace 20.04 by the number of release you want
	# For Eon (19.10)
	$install bison build-essential cmake flex git libedit-dev libllvm7 llvm-7-dev libclang-7-dev python zlib1g-dev libelf-dev
	else
    $install bison build-essential cmake flex git libedit-dev libllvm3.7 llvm-3.7-dev libclang-3.7-dev python zlib1g-dev libelf-dev
	fi   
